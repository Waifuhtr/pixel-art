...
class SpriteReferenceHandler(JobHandler):
    ...
    def run(self, params: SpriteReferenceParams, ctx: JobContext) -> Iterator[Event]:
        ...
        if local_sd.enabled():
            yield log("Generating reference with local SD-Turbo (CPU)", step="start")
            try:
                png_bytes = local_sd.generate_reference_png(ref_prompt)
            except Exception as e:
                yield error(f"Local SD reference generation failed: {e}")
                return
            ref_id = f"ref_{int(time.time())}_{hash(params.prompt) & 0xFFFF:04x}.png"
            storage.save_file(f"references/{ref_id}", png_bytes)
